...
let guesses = []; // Array to store guesses and results
...
function askGuess() {
  ...
  let resultMessage;
  if (guess > random) {
    resultMessage = `Guess: ${guess} - Too high, time taken ${roundDuration} s, remaining ${remainingTime} m.`;
  } else if (guess < random) {
    resultMessage = `Guess: ${guess} - Too low, time taken ${roundDuration} s, remaining ${remainingTime} m.`;
  } else {
    resultMessage = `Guess: ${guess} - Congratulations. You guessed it!`;
    status.textContent = `Number of tries: ${tries}, total time: ${Math.round(
      elapsedTime / 60000
    )} m.`;
